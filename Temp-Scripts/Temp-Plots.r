...
colfoo <-
  col_numeric(colorRamp(c("Blue", "Yellow", "Red"), interpolate = "spline"), dat$Temp)

for (l in 1:length(loggers)) {
  tdat <- subset(dat, Logger = loggers[l])
  pdf(
    paste0('../Temp-Output/ExpPatch_Temp_15min_', loggers[l], '.pdf'),
    width = 4,
    height = 3
  )
  par(
  ...
  ax1.at <- pretty(tdat$DateTime, 20)
  axis(1,
       at = ax1.at,
       labels = format(ax1.at, "%b-%y"),
       las = 2)
  axis(2)
  box(lwd = 1)
...
colfoo <-
  col_numeric(colorRamp(c("Blue", "Yellow", "Red"), interpolate = "spline"), SDay$Temp)

pdf(
  paste0('../Temp-Output/ExpPatch_Temp_Day_Site.pdf'),
  width = 4,
  height = 3
)
par(
